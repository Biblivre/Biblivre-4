...
public class UserDAO extends AbstractDAO {
	...
	public Map<Integer, UserDTO> map(Set<Integer> ids) {
		...
		try {
			...
			sql.append("SELECT U.*, public.array_agg(V.key) as keys, public.array_agg(V.value) as values ");
	...
	public DTOCollection<UserDTO> search(UserSearchDTO dto, int limit, int offset) {
		...
		try {
			...
			StringBuilder sql = new StringBuilder();
			sql.append("SELECT U.*, public.array_agg(V.key) as keys, public.array_agg(V.value) as values FROM users U ");
			...
			if (StringUtils.isNotBlank(dto.getField())) {
				...
				if (StringUtils.isNumeric(query)) {
					...
					countSql.append("AND (U.name ilike ?) ");
				}
			}
	...
	public boolean save(UserDTO user) {
		...
		try {
			...
			if (newUser) {
				...
				sql.append("INSERT INTO users (id, name, type, photo_id, status, created_by) ");
				sql.append("VALUES (?, ?, ?, ?, ?, ?);");
				...
				sql.append("type = ?, photo_id = ?, status = ?, name = ?, modified_by = ?, user_card_printed = ? ");
				sql.append("WHERE id = ?;");
				pst = con.prepareStatement(sql.toString());
