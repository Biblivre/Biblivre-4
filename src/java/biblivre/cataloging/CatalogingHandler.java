...
public abstract class CatalogingHandler extends AbstractHandler {
	...
	//http://localhost:8080/Biblivre4/default?controller=json&module=cataloging.bibliographic&action=search&search_parameters={search_type:%22simple%22,database:%22main%22,material_type:%22all%22,search_terms:[{query:%22machado%22}]}
	...
	//http://localhost:8080/Biblivre4/default?controller=json&module=cataloging.bibliographic&action=paginate&search_id=248&page=20&indexing_group=0
	...
	public void delete(ExtendedRequest request, ExtendedResponse response) {
		...
		if (recordDatabase == RecordDatabase.TRASH) {
			...
			success = bo.moveRecords(new int[]{ dto.getId() }, request.getLoggedUserId(), RecordDatabase.TRASH);
		}		
	...
	public void moveRecords(ExtendedRequest request, ExtendedResponse response) {
		...
		for (int i = 0; i < idList.length; i++) {
			ids[i] = Integer.valueOf(idList[i]);
		}
	...
	//http://localhost:8080/Biblivre4/?controller=download&module=cataloging.export&action=download_export&id={export_id}
