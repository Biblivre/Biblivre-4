...
public class HoldingBO extends RecordBO {
	...
	public boolean save(RecordDTO dto) {
		...
		if (success) {
			...
			try {
				UserDTO udto = UserBO.getInstance(this.getSchema()).get(dto.getCreatedBy());
				LoginDTO ldto = LoginBO.getInstance(this.getSchema()).get(udto.getLoginId());
...
//		}
	...
	public DTOCollection<HoldingDTO> search(String query, RecordDatabase database, boolean lentOnly, int offset, int limit) {
		return this.dao.search(query, database, lentOnly, offset, limit);
	}
	...
	@Override
	public boolean moveRecords(int[] ids, int modifiedBy, RecordDatabase database) {
		throw new RuntimeException("error.invalid_method_call");
	}	
