...
public class HoldingDAO extends AbstractDAO {
	...
	public DTOCollection<HoldingDTO> search(String query, RecordDatabase database, boolean lentOnly, int offset, int limit) {
		...
		try {
			...
			if (!listAll) {
				if (searchId) {				
					sql.append("AND (accession_number = ? OR id = ?) ");
				} else {
					sql.append("AND accession_number = ? ");
				}
			}
			...
			if (!listAll) {
				if (searchId) {				
					countSql.append("AND (accession_number = ? OR id = ?) ");
				} else {
					countSql.append("AND accession_number = ? ");
				}
			}
			...
			if (!listAll) {
				...
				if (searchId) {
					pst.setInt(index, Integer.valueOf(query));
					pstCount.setInt(index++, Integer.valueOf(query));
				}
			}
