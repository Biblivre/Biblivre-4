...
public abstract class AbstractDAO {
	...
	protected final Connection getConnection() throws SQLException {
		...
		*/
		...
		if (this.getSchema() != null) {
			con.createStatement().execute("SET search_path = '" + this.getSchema() + "'");
		}
	...
	public final void fixSequence(String sequence, String tableName, String tableIdColumnName) {
		...
		try {
			...
			String sql = "SELECT setval('" + sequence + "', coalesce((SELECT max(" + tableIdColumnName + ") + 1 FROM " + tableName + "), 1), true);";
			PreparedStatement pst = con.prepareStatement(sql);
