...
public final class SchemaServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		super.init(config);
		try {
			Z3950ServerBO serverBo = new Z3950ServerBO();
			serverBo.startServer();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String controller = ((ExtendedRequest) request).getController();
	...
	protected void processDynamicRequest(HttpServletRequest request, HttpServletResponse response, boolean headerOnly) throws ServletException, IOException {
		...
		// If there is an action but there isn't any controller or module, it's a menu action
		...
		if (controller.equals("jsp")) {
		...
		} else if (controller.equals("media")) {
			...
			} else if (xRequest.getBoolean("force_setup") || Configurations.getBoolean(xRequest.getSchema(), Constants.CONFIG_NEW_LIBRARY)) {
