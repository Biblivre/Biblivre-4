...
public class BackupBO extends AbstractBO {
	...
	public void simpleBackup() {
		...
		if (this.isGlobalSchema()) {
			list.addAll(Schemas.getSchemasList());
		} else {
	...
	public void createBackup(BackupDTO dto) throws IOException {
		...
		for (String schema : schemas.keySet()) {
			...
			if (type == BackupType.FULL || type == BackupType.EXCLUDE_DIGITAL_MEDIA) {
				backup = new File(tmpDir, schema + ".schema.b4b");
				...
				backup = new File(tmpDir, schema + ".data.b4b");
				commands[13] = backup.getAbsolutePath();
			...
			if (!schema.equals(Constants.GLOBAL_SCHEMA)) {	
				if (type == BackupType.FULL || type == BackupType.DIGITAL_MEDIA_ONLY) {
					backup = new File(tmpDir, schema + ".media.b4b");
					commands[13] = backup.getAbsolutePath();
					commands[14] = "--data-only";
					commands[15] = "--table";
					commands[16] = schema + ".digital_media";
					commands[17] = "--blobs";
					this.dumpDatabase(commands);	
		...
		File tmpZip = new File(tmpDir.getAbsolutePath() + ".b4bz");
	...
	public boolean move(BackupDTO dto) {
		...
		//Format: Biblivre Backup 2012-09-08 12h01m22s Full.b4bz
		Formatter formatter = new Formatter(sb);
		formatter.format("Biblivre Backup %1$tY-%1$tm-%1$td %1$tHh%1$tMm%1$tSs %2$s.b4bz", new Date(), StringUtils.capitalize(dto.getType().toString()));
		formatter.close();
