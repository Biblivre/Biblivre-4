...
public class RestoreBO extends AbstractBO {
	...
	public LinkedList<RestoreDTO> list() {
		...
		for (File backup : FileUtils.listFiles(path, new String[]{"b4bz"}, false)) {
	...
	public boolean restore(RestoreDTO dto) {
		...
		for (String schema : dto.getRestoreSchemas().keySet()) {
			steps += FileIOUtils.countLines(new File(tmpDir, schema + ".schema.b4b"));
			steps += FileIOUtils.countLines(new File(tmpDir, schema + ".data.b4b"));

			if (schema != Constants.GLOBAL_SCHEMA) {
				steps += FileIOUtils.countLines(new File(tmpDir, schema + ".media.b4b"));
			}			
		}
	...
	public synchronized boolean restoreBackup(RestoreDTO dto, File path) {
		...
		for (String originalSchemaName : restoreSchemas.keySet()) {			
			if (!deleteSchemas.containsKey(originalSchemaName) && databaseSchemas.contains(originalSchemaName)) {
				...
				postRenameSchemas.put(aux, originalSchemaName);
			}
		}		
		...
		try {
			...
			if (restoreSchemas.containsKey(Constants.GLOBAL_SCHEMA)) {
				...
				this.processRestore(new File(path, Constants.GLOBAL_SCHEMA + ".schema.b4b"), bw);
				
				State.writeLog("Processing data for '" + Constants.GLOBAL_SCHEMA + "'");				
				this.processRestore(new File(path, Constants.GLOBAL_SCHEMA + ".data.b4b"), bw);
				bw.flush();
			}
			...
			for (String schema : restoreSchemas.keySet()) {
				...
				State.writeLog("Processing schema for '" + Constants.GLOBAL_SCHEMA + "'");
				this.processRestore(new File(path, schema + ".schema.b4b"), bw);

				// Restoring database data
				State.writeLog("Processing data for '" + Constants.GLOBAL_SCHEMA + "'");				
				this.processRestore(new File(path, schema + ".data.b4b"), bw);

				// Restoring digital media files
				State.writeLog("Processing media for '" + Constants.GLOBAL_SCHEMA + "'");
				this.processMediaRestore(new File(path, schema + ".media.b4b"), bw);
			...
			for (String originalSchemaName : deleteSchemas.keySet()) {
				...
				if (originalSchemaName != Constants.GLOBAL_SCHEMA) {
			...
			for (String originalSchemaName : restoreSchemas.keySet()) {
				...
				if (finalSchemaName != Constants.GLOBAL_SCHEMA) {
	...
	private void processMediaRestore(File restore, BufferedWriter bw) throws IOException {
		if (restore == null) {
			this.logger.info("===== Skipping File 'null' =====");
			return;
		}
		...
		bw.write("SET search_path = \"" + this.schema + "\", pg_catalog;\n");
