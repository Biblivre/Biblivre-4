...
public class ReportsDAO extends AbstractDAO {
	...
	public SummaryReportDto getSummaryReportData(RecordDatabase database) {
		...
		try {
			...
			while (rs.next()) {
				...
				String location = dataReader.getLocation();
				data[6] = StringUtils.isNotBlank(location) ? location : "";
	...
	public LendingsByDateReportDto getLendingsByDateReportData(String initialDate, String finalDate) {
		...
		try {
			...
			sqlLate.append(" WHERE expected_return_date < to_date(?, 'DD-MM-YYYY') ");
	...
	public TreeMap<String, Set<Integer>> searchAuthors(String authorName, RecordDatabase database, int offset, int limit) {
