	...
	<div id="circulation_search">
		...
		<textarea class="selected_highlight_template template"><!-- 
			<div class="record">
				...
				<label><i18n:text key="circulation.user_field.type" /></label>: {_('circulation.custom.user_type.' + $T.user.type)}<br/>
			</div>
		--></textarea>
				...
				<div class="tab_body biblivre_form_body" data-tab="login">
					...
					<textarea id="biblivre_circulation_permissions_form_template" class="template"><!-- 
						...
						<div class="field">
							...
							<div class="value"><input type="checkbox" name="employee" style="width: auto;" {#if $T.login.employee == 'true'}checked="checked"{#/if}></div>
							<div class="clear"></div>
						</div>
				...
				<div class="tab_extra_content biblivre_form" id="permissions" data-tab="login">
					<%
						...
						for (AuthorizationPointTypes atp : AuthorizationPointTypes.values()) {
							...
							out.println("<div><input type=\"checkbox\" value=\"" + atp + "\" name=\"permissions\" id=\"permission_" + atp + "\" />&#160;<label for=\"permission_" + atp + "\">" + translations.getText("administration.permissions.items." + atp.toString().toLowerCase()) + "</label></div>");
						}
			...
			<div class="simple_search submit_on_enter">
				...
				<div class="buttons">
					...
					<select name="field" class="combo">
						...
						<c:forEach var="field" items="<%= UserFields.getFields((String) request.getAttribute(\"schema\")) %>" >
					...
					{#foreach $T.data as record}
						<div class="result {#cycle values=['odd', 'even']}" rel="{$T.record.id}">
							...
							<div class="record">
								...
								<label><i18n:text key="circulation.user_field.type" /></label>: {_('circulation.custom.user_type.' + $T.record.user.type)}<br/>
								{#if $T.record.login}<label><i18n:text key="circulation.user_field.login" /></label>: {$T.record.login.login}<br/>{#/if}
							</div>
							<div class="clear"></div>
						</div>
