	...
	<div id="circulation_user">
		...
		<textarea class="selected_highlight_template template"><!-- 
			...
			<div class="record">
				...
				{#if $T.type}<label><i18n:text key="circulation.user_field.type" /></label>: {_('circulation.custom.user_type.' + $T.type)}<br/>{#/if}
					...
					{#foreach $T.data as record}
						<div class="result {#cycle values=['odd', 'even']}" rel="{$T.record.id}">
							...
							<div class="record">
								...
								<label><i18n:text key="circulation.user_field.type" /></label>: {_('circulation.custom.user_type.' + $T.record.type)}<br/>
