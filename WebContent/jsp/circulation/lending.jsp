					...
					{#foreach $T.data as record}
						<div class="result {#cycle values=['odd', 'even']}" rel="{$T.record.id}">
							...
							<div class="record">
								...
								{#if $T.record.biblio.shelf_location || $T.record.holding.location_d}
									<label><i18n:text key="search.bibliographic.shelf_location" /></label>: {$T.record.biblio.shelf_location || ''} {$T.record.holding.location_d || ''}<br/>
								{#/if}
								...
								{#if $T.record.lending !== undefined}
									<div class="user_lending">				
										...
										<label><i18n:text key="circulation.lending.expected_return_date" /></label>: {_d($T.record.lending.expectedReturnDate, 'D')}<br/>
