					...
					<fieldset>
						...
						<textarea id="biblivre_attachments_template" class="template"><!-- 
							{#if !$T.attachments || $T.attachments.length == 0}
								<div class="center"><strong><i18n:text key="cataloging.bibliographic.no_attachments" /></strong></div>
							{#/if}
							{#foreach $T.attachments as attachment}
								<li><a href="{$T.attachment.uri}" target="_blank">{$T.attachment.name}</a> <a class="xclose" onclick="CatalogingInput.removeAttachment(this, '{$T.attachment.name}', '{$T.attachment.uri}');">&times;</a></li>
							{#/for}
						--></textarea>
